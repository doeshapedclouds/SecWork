...
The core path: Exploit.
               in a method an every day user would see as benign. Or a principal engineer would say is "Odd, but not necessarily concerning on its own".
...
- Several findings were thought to be malicious, then decided benign, but seemingly are indicative of the transitional state of this process and not a false positive.
- Analysis provenance is flagged where client-side tools may have been operating in a compromised execution context. Though also follow the above disclaimer of transitional state.
- This is a living document — findings are updated as new evidence is gathered. Its hard to keep it clean with its depth.

## Disclosure

Intended for review by Apple Security eventually. Currently just architecting.
